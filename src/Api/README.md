...
- **Tools reach the API over HTTP** (ADR 002), carrying the caller's `Authorization` header via
  `ForwardCallerIdentityHandler`. This handler enforces the guarantee that the assistant can never do
  more than the logged-in user. Removing it makes every tool result a 401.
