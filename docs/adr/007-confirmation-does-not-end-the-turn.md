...
Tool calls are executed by `Microsoft.Extensions.AI`'s `FunctionInvokingChatClient`, which owns the loop: it invokes the function, appends the result to the conversation and goes straight back to the model. `ChatOrchestrator` only *observes* `FunctionCallContent` and `FunctionResultContent` as they stream past. The orchestrator cannot stop the loop without replacing that middleware or throwing from a tool. A thrown tool call reaches the model as a failure, which misrepresents a proposal.
...
**The turn continues.** A gated write returns a structured result containing `status: "pending_approval"`,
the action id, the server's summary, and the deadline. The model writes one short line about it, and
the `approval_required` SSE event puts the card beside that line. Nothing has been written to the
database.
...
- **A proposal spends the per-turn write budget, exactly as an execution does.** If only executed writes counted, "cancel every invoice" would execute nothing and still produce eight approval cards. The user would see a wall of pre-filled Approve buttons. The budget is spent by the intent, not the outcome; the second proposal in a turn is `blocked`.
