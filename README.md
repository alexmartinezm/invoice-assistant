# AI assistant in .NET: tool calling, write safety, evals and cost tracking
...
**invoice-assistant** is an AI assistant embedded in a demo invoicing app. It is a reference repo covering tool calling, write safety, evaluation, and usage tracking.
...
2. **Server-side write gate:** writes are proposed, not executed. Human confirmation uses `PendingAction`, policy lives in `policies.json`, and per-turn limits stop prompt injection from turning into bulk changes.
3. **Evals in CI:** prompt regressions break the pipeline. The repo includes an xUnit harness, declarative YAML cases, and [a one-line regression you can run yourself](docs/ai/evaluation.md#proving-a-regression-turns-it-red).
...
**The system prompt can ask for good behavior; only the server can guarantee it.** The prompt is UX, the policy is security. Key decisions are documented as ADRs in [`docs/adr/`](docs/adr/).
...
For development, use two terminals with hot reload:
...
which variables it wants, so you can read and run the repo before signing up to a provider.
...
Behind a TLS-terminating proxy, set `ASSISTANT_API_BASE_URL` so the assistant can call its own API
over HTTP. Set the daily spend cap before exposing the URL. Both settings, plus a managed-database
variant and a troubleshooting table, are in [`docs/deployment.md`](docs/deployment.md).

## What a conversation costs

Every call to the model is metered inside the tool-calling loop, not once per turn, because a turn
that uses tools makes several calls. Each one is recorded with its tokens,
...
nothing about a scraper with many IPs. The euro cap makes a public demo safe to leave running with a
real key in it. The reasoning, including what happens to a model with no configured
price, is in [ADR 008](docs/adr/008-cost-accounting-and-the-spend-kill-switch.md).

Traces complete the picture. The chat footer prints each turn's trace id, and
[`docs/deployment.md`](docs/deployment.md#seeing-the-traces) has the two ways to make that id lead
somewhere: a console exporter or any OTLP collector. The same section describes the spans and meter.
...
The gate is easy to show working. Durable actions make failures halfway through an operation visible.
Everything below runs against `docker compose up` with
...
6. **Optionally, change a different draft after proposing something against it**, then approve. The
   write is refused with `resource_changed` because the approval is bound to the invoice revision it
   was proposed against. The card says the invoice moved on.

The demo distinguishes "approved" from "done" and records the state reached for each action.
...
**Out of scope for v1:** RAG, cross-session memory, multi-agent workflows, real multi-tenancy, legal invoicing (Verifactu, etc.), and i18n.
